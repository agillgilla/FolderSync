...
namespace FolderSync
{
    ...
    {
        public static BitmapSource GetIconAsBitmapSource(string fileExtension, Size size) {
            IntPtr hIcon = GetJumboIcon(GetIconIndex("*" + fileExtension));
            ...
            using (Icon ico = (Icon)Icon.FromHandle(hIcon).Clone()) {
                ...
                Bitmap iconBitmap = ico.ToBitmap();

                float xScale = (float) size.Width / iconBitmap.Width;
                float yScale = (float) size.Height / iconBitmap.Height;

                var bitmapData = iconBitmap.LockBits(
                    new Rectangle(0, 0, iconBitmap.Width, iconBitmap.Height),
                    ImageLockMode.ReadOnly, iconBitmap.PixelFormat);

                var iconBitmapSource = BitmapSource.Create(
                    bitmapData.Width, bitmapData.Height,
                    iconBitmap.HorizontalResolution, iconBitmap.VerticalResolution,
                    PixelFormats.Bgra32, null,
                    bitmapData.Scan0, bitmapData.Stride * bitmapData.Height, bitmapData.Stride);

                iconBitmap.UnlockBits(bitmapData);

                bitmapSource = new TransformedBitmap(iconBitmapSource, new ScaleTransform(xScale, yScale));
            }
